---
name: openidea
description: Kelola siklus hidup ide produk end-to-end secara file-based — dari visi produk (BRIEF.md), penangkapan ide mentah, evaluasi & kategorisasi, penyusunan roadmap, sampai draft spec teknis (OpenSpec). Trigger otomatis saat user menyebut "openidea", "capture ide", "roadmap produk", "proposal client", "BRIEF produk", atau menjalankan command /oi:init, /oi:capture, /oi:evaluate, /oi:plan, /oi:spec-draft, /oi:compile, /oi:export, /oi:spec-audit.
---

# OpenIdea

Sistem manajemen ide produk file-based. State hidup di markdown di bawah folder `openidea/` pada root project. Skill ini adalah rujukan tunggal untuk schema file, prinsip desain, dan prosedur regenerasi index yang dipakai bersama oleh semua command `/oi:*`.

Command individual ada di `.claude/commands/oi/*.md`. Tiap command file self-contained (prasyarat + proses + output), tapi tetap merujuk ke dokumen ini untuk detail schema — jangan duplikasi schema di command file, cukup pointer ke section terkait di sini.
...
- **Independen dari OpenSpec** — OpenIdea tidak pernah menulis ke `openspec/changes/` atau `openspec/specs/` secara langsung kecuali lewat `/oi:spec-draft`. Tidak bergantung pada skill `openspec-drafter` generic.
...
├── .claude/skills/openidea/     ← skill ini sendiri
├── .claude/commands/oi/         ← command /oi:*
└── openspec/                    ← TIDAK PERNAH DISENTUH langsung, kecuali via /oi:spec-draft
    ├── changes/
    └── specs/
```

Semua path folder relatif ke root `openidea/` di root project. `openspec/` milik tooling lain — OpenIdea hanya *membaca* dari situ (misal cek folder ada/belum), tidak pernah menulis kecuali lewat `/oi:spec-draft`.
...
Body (naratif, ditulis oleh `/oi:init`):
...
created: 2026-07-16
updated: 2026-07-20
source: discovery/2026-07-16-session.md     # opsional, dari /oi:capture mode import
...
- Nama milestone **harus** identik dengan nama folder `openspec/changes/<nama>/` nanti. Slug dari nama milestone harus unik (divalidasi di `/oi:plan`).
...
|---|---|---|
| `/oi:init` | — | `BRIEF.md` |
| `/oi:capture` | `BRIEF.md` ada | `ideas/<slug>.md` baru, status `captured` |
| `/oi:evaluate` | ≥1 ide `captured` | update frontmatter ide, pindah ke `archive/` jika final |
| `/oi:plan` | ≥1 ide `ready` | `ROADMAP.md` |
| `/oi:spec-draft` | `openspec/` ada, milestone target punya ≥1 ide `ready` | `openspec/changes/<milestone>/{proposal,design,tasks}.md`, ide → `promoted` |
| `/oi:compile` | ada ide `ready` di `ROADMAP.md` | `proposals/vN.md` baru |
| `/oi:export` | — | `exports/<milestone-slug>-vN.md` |
| `/oi:spec-audit` | folder `openspec/changes/<milestone>` ada | laporan drift (read-only) |

Detail proses tiap command: baca file command-nya masing-masing di `.claude/commands/oi/`.

Catatan: `/oi:release` dan `/oi:list` **bukan** command terpisah.
...
| Non-fitur (chore) | `type: chore` pakai 5Q sederhana, dikecualikan dari `/oi:compile` (client-facing), tetap disertakan di `/oi:export` (paket teknis). |

## 7. Prosedur Regenerasi `ideas/INDEX.md`

Dipanggil oleh `/oi:capture`, `/oi:evaluate`, `/oi:plan`, `/oi:spec-draft` setelah mengubah state ide — dan boleh dipanggil standalone kapan saja untuk "menyembuhkan" inkonsistensi.
